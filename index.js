                                ...
                                if (parseInt(resp2[0].coins) >= parseInt(resp[parseInt(interaction.options.get("reward").value) - 1].cost)) {
                                    ...
                                    if (parseInt(interaction.options.get("reward").value) === 6) {
                                        client.guilds.fetch("842146071626514462").then(guild => guild.members.fetch(interaction.user.id).then(member => member.roles.cache.forEach(role => {
                                            if (role.id === "852675470319026177") {
                                                console.log("You already are part of the Children of Epik.");
                                                return interaction.reply("You already are part of the Children of Epik.");
                                            }
                                        })));
