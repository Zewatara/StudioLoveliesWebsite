                            ...
                            if (success2) {
                                ...
                                if (parseInt(resp2[0].coins) >= parseInt(resp[parseInt(interaction.options.get("reward").value) - 1].cost)) {
                                    ...
                                    if (parseInt(interaction.options.get("reward").value) === 6) {
                                        if (client.guilds.fetch("842146071626514462").then(guild => guild.members.fetch(interaction.user.id).then(member => member.roles.cache.has("852675470319026177")))) return interaction.reply("You already are a Children of Epik.");
                                        else {
